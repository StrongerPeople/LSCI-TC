...
class CLIP(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            # soft-label
            sim_i2t_targets = (1 - soft_alpha) * F.softmax(sim_i2t_m, dim=1) + soft_alpha * sim_targets
            sim_t2i_targets = (1 - soft_alpha) * F.softmax(sim_t2i_m, dim=1) + soft_alpha * sim_targets
        ...
        itc_loss = (WeightsoftCEloss(logits_per_image, sim_i2t_targets) + WeightsoftCEloss(logits_per_text, sim_t2i_targets, text_to_image=True)) / 2.0
        # itc_loss = (nn.CrossEntropyLoss()(logits_per_image, sim_i2t_targets) + nn.CrossEntropyLoss()(logits_per_text, sim_t2i_targets)) / 2.0
        # itc_loss = (Focal(gamma=1.0)(logits_per_image, sim_i2t_targets) + Focal(gamma=1.0)(logits_per_text, sim_t2i_targets)) / 2.0
        total_loss = itc_loss
        bs = image.size(0)
        with torch.no_grad():
